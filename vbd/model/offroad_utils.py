...
def compute_signed_distance_to_nearest_road_edge_point(
    ...
    if square_distances.shape[-1] == 0:
        # handle gracefully, e.g., return a default or NaN
        return torch.full(square_distances.shape[:-1], float("nan"), device=square_distances.device)
    else:
