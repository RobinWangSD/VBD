...
def plot_ego(
    ...
    visualization.plot_all_trajectory(
        ax, traj, is_controlled, traj_pred, style_dict, control_list,
        plot_freq, time_idx=10, 
        indices=indices, past_traj_length = past_traj_length,
        is_ego = is_ego, is_adv = is_adv,
    )  # pytype: disable=wrong-arg-types  # jax-ndarray
    ...
    if t is None:
        t = (current_state.timestep-10)/10
    # if font_size>0:
    #     ax.text(origin_x - 0.9*dx, origin_y + 0.85*dx, f"t=1s", fontsize=18)
    ...
    for i in range(10,50):
        ...
        if filename is not None:
            plt.savefig(filename,
                        ...
                        transparent=False,
                        pad_inches=0.02)
        imgs.append(mediapy.resize_image(visualization.utils.img_from_fig(fig), img_size))
